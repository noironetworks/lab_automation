...
ENV no_proxy="${no_proxy}"
    ...
    if container_name == 'neutron-server':
        source_dirs = ['custom_kolla', 'db_migration']

        # Copy each directory to the temporary directory
        for src_dir in source_dirs:
            if os.path.exists(src_dir):  # Check if source directory exists
                dest_path = os.path.join(build_dir, os.path.basename(src_dir))
                shutil.copytree(src_dir, dest_path, dirs_exist_ok=True)
                print(f"Successfully copied {src_dir} to {dest_path}")
            else:
                print(f"Source directory {src_dir} does not exist")

        print("Adding kolla_extend_start to container = %s" % container_name)
        blob = blob + "COPY ./custom_kolla/kolla_extend_start /usr/local/bin/kolla_extend_start \n"
        blob = blob + "RUN chmod +x /usr/local/bin/kolla_extend_start \n"
        blob = blob + "COPY ./db_migration/aci_db_sync /usr/local/bin/ \n"
        blob = blob + "RUN chmod +x /usr/local/bin/aci_db_sync \n"
...
gpgcheck=0
	...
	'neutron-openvswitch-agent': {
        ...
        'opflex-agent': {
            ...
            "packages": [],
            "run_cmds": ["yum --releasever={} -y install opflex-agent nftables opflex-agent-renderer-openvswitch noiro-openvswitch-lib noiro-openvswitch-otherlib ethtool python3-neutron-opflex-agent python3-apicapi python3-openstack-neutron-gbp lldpd os-net-config".format(rhel_version)],
