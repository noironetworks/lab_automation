...
FAB_NAME=$(curl -X GET http://noirolab.cisco.com:5000/api/fabric/${FAB_NO} | jq -r .name)
...
if [ "${UNDERCLOUD_TYPE}" = "${DIRECTOR}" ]; then
    ...
    KEYSTONE_IP=`ssh -o StrictHostKeyChecking=no ${OVERCLOUD_USER}@${CTRLR_INT_IP} "sudo egrep auth_url /etc/neutron/neutron.conf"  | grep v3 | awk -F':' '{print $2}' | cut -c3-`
    ...
    for host in $(cat routes.sh | awk '{print $5}'); do
        ssh -o StrictHostKeyChecking=no ${OVERCLOUD_USER}@$host "sudo route add -net ${GW1_IP} netmask 255.255.255.0 gateway 1.121.${MGMT_VLAN}.1 dev vlan${MGMT_VLAN}".  
    done
    sudo route add -net 1.121.${MGMT_VLAN}.0 netmask 255.255.255.0 gateway ${GW1_IP}
    ssh -o StrictHostKeyChecking=no ${OVERCLOUD_USER}@${CTRLR_INT_IP} "sudo cp ~/${RCFILE} /root/" >> /dev/null
    # Establish ssh logins
    for CIP in $(source stackrc && nova list | grep controller | awk -F'|' '{print $7}' | cut -c11-); do 
