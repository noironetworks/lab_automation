...
sudo yum install ./tripleo-ciscoaci-16.1-1052.noarch.rpm
# 5. Log in to the upstream registry
sudo podman login registry.redhat.io
# 6. Build the OSP16 cisco containers:
sudo /opt/ciscoaci-tripleo-heat-templates/tools/build_openstack_aci_containers.py -z openstack-ciscorpms-repo-16.1-1052.tar.gz
# 7. Build the OSP15 (Stein) transitional containers (--force is needed because of md5sum mismatch):
sudo /opt/ciscoaci-tripleo-heat-templates/tools/build_transitional_aci_containers.py -z openstack-ciscorpms-repo-15.0-1053.tar.gz --force
