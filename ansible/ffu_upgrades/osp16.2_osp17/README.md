...
$ ansible-playbook -i inventory.txt step2.5.yaml
$ ansible-playbook -i inventory.txt step2.6.yaml
$ ansible-playbook -i inventory.txt step2.8.yaml
</code></pre>
...
11. Perform the undercloud LEAPP
<pre><code>$ ansible-playbook -i inventory.txt step9.1.yaml
...
12. Run the playbook to perform the LEAPP upgrade of the control plane

<pre><code>$ ansible-playbook -i inventory.txt step10.1.yaml
</code></pre>

12. Run the playbook to perform the LEAPP upgrade of the computes

Note: at this point, the old inventory.txt will no longer work, as the user is heat-admin, but now should be tripleo-admin.

<pre><code>$ ansible-playbook -i /home/stack/overcloud-deploy/overcloud/tripleo-ansible-inventory.yaml step11.2.yaml
</code></pre>

12. Fix SSH to overcloud nodes

Until Red Hat BZ#2265054 is fixed, we will have to fix the SSH keys to the overcloud node. Run this playbook to fix them:
<pre><code>$ ansible-playbook -i inventory.txt fix-ssh-keys.yaml
</code></pre>


13. Run the playbooks to clean up post-LEAPP
...
14. Compress the Horizon templates
