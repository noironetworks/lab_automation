...
$HOME/openstack_automation_scripts/install_openstack/focal-get-machines
...
export CONFIG=$HOME/openstack_automation_scripts/install_openstack/focal-yoga/yoga-opflex-sfc.yaml

juju deploy --config=$CONFIG ch:bionic/percona-cluster mysql --to lxd:"$controller_id"
juju config mysql max-connections=1300

juju deploy --config=$CONFIG cs:rabbitmq-server --to lxd:"$controller_id"
$HOME/openstack_automation_scripts/install_openstack/app-status

juju deploy --config=$CONFIG cs:keystone --to lxd:"$controller_id"
juju add-relation keystone:shared-db mysql:shared-db
juju deploy --config=$CONFIG cs:glance --to lxd:"$controller_id"
...
juju deploy $HOME/charms/charm-neutron-api-plugin-aci --series focal --config=$CONFIG neutron-api-plugin-aci
...
juju deploy --series focal --config=$CONFIG nova-cloud-controller --to lxd:"$controller_id"
...
juju deploy --series focal --config=$CONFIG cs:placement --to lxd:"$controller_id"
...
juju deploy --series focal --config=$CONFIG cs:nova-compute --to "$compute01_id"
juju add-relation nova-compute:amqp rabbitmq-server:amqp
juju add-relation nova-compute:cloud-compute nova-cloud-controller:cloud-compute
juju add-relation nova-compute:image-service glance:image-service

juju deploy --series focal --config=$CONFIG cs:openstack-dashboard --to "$dashboard_id"
...
juju deploy $HOME/charms/charm-neutron-aci-opflex --series focal --config=$CONFIG neutron-aci-opflex
