...
export CONFIG=$HOME/openstack_automation_scripts/install_openstack/focal-wallaby/wallaby-opflex-sfc.yaml

juju deploy --config=$CONFIG cs:bionic/percona-cluster mysql --to lxd:"$controller_id"
#juju deploy -n 3  ch:focal/mysql-innodb-cluster mysql --to lxd:1
#juju deploy --config=$CONFIG --channel 8.0/stable ch:mysql-innodb-cluster mysql --to lxd:1
#juju deploy --config=$CONFIG cs:mysql-innodb-cluster mysql --to lxd:1
...
juju deploy --config=$CONFIG --series focal nova-cloud-controller --to lxd:"$controller_id"
...
juju deploy --config=$CONFIG nova-compute --series focal --to "$compute01_id"
