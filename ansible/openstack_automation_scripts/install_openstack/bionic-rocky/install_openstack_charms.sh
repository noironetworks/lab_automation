...
juju bootstrap fab8-cloud --bootstrap-series bionic --to juju --config no-proxy=$no_proxy --config  http-proxy=$http_proxy --config https-proxy=$https_proxy
...
export CONFIG=$HOME/openstack_automation_scripts/install_openstack/bionic-rocky/rocky-opflex-sfc.yaml

juju deploy --config=$CONFIG cs:xenial/mysql --to lxd:"$controller_id"
...
# juju deploy --config=$CONFIG $HOME/charms/charm-nova-compute --series bionic --to "$compute01_id"
juju deploy  --config=$CONFIG nova-compute --channel rocky/candidate --series bionic --to "$compute01_id"
