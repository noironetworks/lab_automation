...
#juju bootstrap fab8-cloud --bootstrap-series focal --to juju --config no-proxy=$no_proxy --config  http-proxy=$http_proxy --config https-proxy=$https_proxy
juju bootstrap fab8-cloud --bootstrap-series focal --to juju --config no-proxy=$no_proxy --config  http-proxy=$http_proxy --config https-proxy=$https_proxy --config  snap-http-proxy=$http_proxy --config snap-https-proxy=$https_proxy
...
export CONFIG=$HOME/openstack_automation_scripts/install_openstack/focal-xena/xena-opflex-sfc.yaml
...
juju deploy --config=$CONFIG cs:focal/rabbitmq-server --to lxd:"$controller_id"

$HOME/openstack_automation_scripts/install_openstack/app-status
juju deploy --config=$CONFIG cs:focal/keystone --to lxd:"$controller_id"
juju add-relation keystone:shared-db mysql:shared-db

juju deploy --config=$CONFIG cs:focal/glance --to lxd:"$controller_id"
juju add-relation glance:identity-service keystone:identity-service
juju add-relation glance:shared-db mysql:shared-db

juju deploy $HOME/charms/charm-neutron-api  --series focal --config=$CONFIG neutron-api --to "$controller_id"
...
juju deploy $HOME/charms/charm-neutron-gateway --series focal --config=$CONFIG neutron-gateway --to "$gateway_id"
juju add-relation neutron-gateway:amqp rabbitmq-server:amqp
juju add-relation neutron-gateway:neutron-plugin-api neutron-api:neutron-plugin-api


juju deploy --config=$CONFIG cs:focal/nova-cloud-controller --to lxd:"$controller_id"
...
juju deploy --config=$CONFIG cs:focal/placement --to lxd:"$controller_id"
...
#juju deploy --config=$CONFIG cs:focal/nova-compute --to 3
# juju deploy --config=$CONFIG $HOME/charms/charm-nova-compute --series focal --to "$compute01_id"
juju deploy  --config=$CONFIG nova-compute --channel xena/candidate --series focal --to "$compute01_id"
juju add-relation nova-compute:amqp rabbitmq-server:amqp
juju add-relation nova-compute:cloud-compute nova-cloud-controller:cloud-compute
juju add-relation nova-compute:image-service glance:image-service

juju deploy --config=$CONFIG cs:focal/openstack-dashboard --to "$dashboard_id"
juju add-relation openstack-dashboard:identity-service keystone:identity-service

juju deploy $HOME/charms/charm-openstack-dashboard-plugin-gbp  --config=$CONFIG openstack-dashboard-plugin-gbp
...
juju deploy $HOME/charms/charm-openstack-dashboard-plugin-gbp --series focal --config=$CONFIG openstack-dashboard-plugin-gbp
juju add-relation openstack-dashboard-plugin-gbp openstack-dashboard
