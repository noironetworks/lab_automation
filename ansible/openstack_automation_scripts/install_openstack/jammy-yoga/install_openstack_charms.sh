#set -x
juju bootstrap fab8-cloud --bootstrap-series=jammy --to juju --config no-proxy=$no_proxy --config http-proxy=$http_proxy --config https-proxy=$https_proxy --config  snap-http-proxy=$http_proxy --config snap-https-proxy=$https_proxy
$HOME/openstack_automation_scripts/install_openstack/jammy-get-machines
...
export CONFIG=$HOME/openstack_automation_scripts/install_openstack/jammy-yoga/yoga-opflex-sfc.yaml

juju deploy --config=$CONFIG ch:bionic/percona-cluster mysql --to lxd:"$controller_id"
#juju deploy -n 3 --to lxd:2,lxd:2,lxd:2 --config=$CONFIG mysql-innodb-cluster --channel 8.0/stable mysql
#juju deploy mysql-router keystone-mysql-router
#juju deploy --config=$CONFIG ch:keystone --to lxd:2
#juju integrate keystone-mysql-router:db-router mysql:db-router
#juju integrate keystone-mysql-router:shared-db keystone:shared-db
juju config mysql max-connections=1300
juju deploy --config=$CONFIG ch:rabbitmq-server --to lxd:"$controller_id"
$HOME/openstack_automation_scripts/install_openstack/app-status

juju deploy --config=$CONFIG ch:keystone --to lxd:"$controller_id"
juju add-relation keystone:shared-db mysql:shared-db

juju deploy --config=$CONFIG ch:glance --to lxd:"$controller_id"
juju add-relation glance:identity-service keystone:identity-service
juju add-relation glance:shared-db mysql:shared-db

juju deploy $HOME/charms/charm-neutron-api --series jammy --config=$CONFIG neutron-api --to "$controller_id"
juju add-relation neutron-api:amqp rabbitmq-server:amqp
juju add-relation neutron-api:identity-service keystone:identity-service
juju add-relation neutron-api:shared-db mysql:shared-db
juju deploy $HOME/charms/charm-neutron-api-plugin-aci --series jammy --config=$CONFIG neutron-api-plugin-aci
...
juju deploy $HOME/charms/charm-neutron-gateway --series jammy --config=$CONFIG neutron-gateway --to "$gateway_id"
juju add-relation neutron-gateway:amqp rabbitmq-server:amqp
juju add-relation neutron-gateway:neutron-plugin-api neutron-api:neutron-plugin-api


juju deploy --series jammy --config=$CONFIG ch:nova-cloud-controller --to lxd:"$controller_id"
#juju deploy nova-cloud-controller --channel yoga/stable --to lxd:1
...
juju deploy --series jammy --config=$CONFIG ch:placement --to lxd:"$controller_id"
...
juju deploy --series jammy --config=$CONFIG ch:nova-compute --to "$compute01_id"
#juju deploy nova-compute --channel yoga/stable --to 3
juju add-relation nova-compute:amqp rabbitmq-server:amqp
juju add-relation nova-compute:cloud-compute nova-cloud-controller:cloud-compute
juju add-relation nova-compute:image-service glance:image-service

juju deploy --series jammy --config=$CONFIG ch:openstack-dashboard --to "$dashboard_id"
juju add-relation openstack-dashboard:identity-service keystone:identity-service

juju deploy $HOME/charms/charm-openstack-dashboard-plugin-gbp  --series jammy --config=$CONFIG openstack-dashboard-plugin-gbp
...
juju deploy $HOME/charms/charm-neutron-aci-opflex --series jammy --config=$CONFIG neutron-aci-opflex
...
$HOME/openstack_automation_scripts/install_openstack/app-status
#juju deploy $HOME/charms/charm-openstack-dashboard-plugin-aci --series focal --config=$CONFIG openstack-dashboard-plugin-aci
