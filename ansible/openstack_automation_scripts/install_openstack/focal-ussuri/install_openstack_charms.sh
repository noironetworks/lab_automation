...
export CONFIG=$HOME/openstack_automation_scripts/install_openstack/focal-ussuri/ussuri-opflex-sfc.yaml
...
juju deploy --config=$CONFIG cs:nova-cloud-controller --to lxd:"$controller_id"
...
juju deploy --config=$CONFIG cs:placement --to lxd:"$controller_id"
...
#  juju deploy --series focal --config=$CONFIG cs:focal/nova-compute --to "$compute01_id"
juju deploy --config=$CONFIG cs:nova-compute --to "$compute01_id"
#juju deploy --config=$CONFIG $HOME/charms/charm-nova-compute --series focal --to 3
juju add-relation nova-compute:amqp rabbitmq-server:amqp
juju add-relation nova-compute:cloud-compute nova-cloud-controller:cloud-compute
juju add-relation nova-compute:image-service glance:image-service

juju deploy --config=$CONFIG cs:openstack-dashboard --to "$dashboard_id"
